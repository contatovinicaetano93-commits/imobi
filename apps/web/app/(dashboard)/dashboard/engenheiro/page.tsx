  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Fila de Visitas</h1>
          <p className="text-sm text-gray-600 mt-1">Gerencie suas inspeções de obra</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={kpi.label} className={`rounded-2xl p-6 border ${kpi.color}`}>
      ...
      <section>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Próximas Visitas</h2>
        {agendadas.length === 0 ? (
          ...
          <div className="space-y-3">
            {agendadas.slice(0, 3).map((visita) => (
              <a
                ...
                className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 hover:shadow-md hover:border-brand-200 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{visita.obra.nome}</h3>
          ...
          <div className="space-y-3">
            {iniciadas.map((visita) => (
              <a
                key={visita.visitaId}
                href={`/dashboard/engenheiro/${visita.visitaId}`}
                className="bg-white rounded-xl border border-yellow-200 bg-yellow-50 p-4 sm:p-6 hover:shadow-md transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{visita.obra.nome}</h3>
          ...
          <div className="space-y-3">
            {concluidas.slice(0, 5).map((visita) => (
              <a
                key={visita.visitaId}
                href={`/dashboard/engenheiro/${visita.visitaId}`}
                className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 hover:shadow-md transition-all opacity-75"
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-600 text-sm sm:text-base">{visita.obra.nome}</h3>
