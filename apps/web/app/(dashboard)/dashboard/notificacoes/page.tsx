...
export default function NotificacoesPage() {
  ...
  const panels = useMemo(
    () => [
      ...(naoLidasCount > 0
        ? [{ id: "notif-resumo", priority: "critical" as const }]
        : []),
      { id: "notif-lista", priority: "primary" as const },
    ],
    [naoLidasCount],
  );
        ...
        <>
          ...
          {naoLidasCount > 0 && (
            <PanelSection
              ...
              priority="critical"
              badge={naoLidasCount}
              summary={`${naoLidasCount} aguardando sua atenção`}
              urgency="warning"
            >
              <p className="text-sm text-amber-800 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
                Revise as notificações pendentes para não perder prazos de obra, crédito ou documentação.
              </p>
            </PanelSection>
          )}
          ...
          <PanelSection
            ...
            icon={<Bell className="w-4 h-4 text-[#1B4FD8]" />}
            priority="primary"
            badge={filtradas.length || undefined}
            summary={
              tab === "nao-lidas"
                ? `${filtradas.length} não lida(s)`
                    ...
                    <>
                      <span
                        className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold ${
                          naoLidasCount > 0 ? "bg-[#1B4FD8] text-white" : "bg-gray-200 text-gray-500"
                        }`}
                      >
                        {naoLidasCount}
                      </span>
                ...
                {filtradas.map((n) => {
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border shadow-sm p-4 sm:p-5 flex items-start gap-4 transition-all duration-150 ${
                        n.link ? "cursor-pointer hover:shadow-md hover:bg-gray-50/50" : ""
                      } ${n.lida ? "border-gray-100 bg-white" : "border-[#1B4FD8]/20 bg-blue-50/20"}`}
                    >
