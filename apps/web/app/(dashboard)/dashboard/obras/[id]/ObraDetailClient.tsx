...
export default function ObraDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    Promise.all([
      obrasApi.buscar(id),
      obrasApi.progresso(id).catch(() => 0),
    ])
      .then(([o, p]) => { setObra(o); setProgresso(p as number); })
      .catch((err: Error) => setError(err?.message ?? "Erro ao carregar obra"))
    ...
    return (
      <div className="space-y-8 max-w-4xl">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
          <p className="text-gray-500">Carregando detalhes da obra...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !obra) {
    return (
      <div className="space-y-8 max-w-4xl">
        <p className="text-red-500">{error ?? "Obra não encontrada."}</p>
      </div>
    );
  }
