        ...
        <div className="border-t border-gray-100 p-4 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <input
                ...
                value={simulacao.valor}
                onChange={(e) => setSimulacao({ ...simulacao, valor: Number(e.target.value) })}
                className="flex-1 px-2 py-1 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent"
              />
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-brand-600">{simulacao.prazo} meses</span>
            </div>
            <input
              type="range"
              min="1"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-brand-600">{simulacao.taxa.toFixed(2)}%</span>
            </div>
          ...
          {/* Resultado */}
          <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 space-y-2 mt-4">
            <p className="text-xs text-brand-600 font-semibold">PARCELA MENSAL ESTIMADA</p>
            <p className="text-2xl font-bold text-brand-900">{formatarBRL(calculoParcela)}</p>
            <div className="grid grid-cols-2 gap-2 text-xs pt-3 border-t border-brand-100">
              <div>
                <p className="text-brand-700/70">Total a Pagar</p>
                <p className="font-semibold text-brand-900">
                  {formatarBRL(calculoParcela * simulacao.prazo)}
                </p>
              </div>
              <div>
                <p className="text-brand-700/70">Juros Totais</p>
                <p className="font-semibold text-brand-900">
          ...
          <a
            ...
            className="block w-full text-center px-4 py-2 text-sm font-semibold text-brand-600 hover:text-brand-700 border-t border-gray-100 pt-4"
          >
