...
export const DASHBOARD_NAV: DashboardNavItem[] = [
  { label: "Início", href: TOMADOR_HOME, icon: Home, roles: ["CONSTRUTOR", "TOMADOR"], section: "geral", mobileTab: true },
  { label: "Obras", href: "/dashboard/obras", icon: HardHat, roles: ["TOMADOR", "CONSTRUTOR"], funcao: "obras", mobileTab: true },
  { label: "Perfil", href: "/dashboard/perfil", icon: User, roles: ["TOMADOR", "CONSTRUTOR"], mobileTab: true },
  ...
  { label: "Monitorar etapas", href: "/dashboard/gestor/etapas", icon: ShieldCheck, roles: ["GESTOR", "GESTOR_FUNDO"], funcao: "gestor" },
  { label: "Monitorar KYC", href: "/dashboard/gestor/kyc", icon: ShieldCheck, roles: ["GESTOR", "GESTOR_FUNDO"], funcao: "kyc", mobileTab: true },
  ...
  { label: "Painel", href: "/dashboard/comercial", icon: BarChart3, roles: ["COMERCIAL", "PARCEIRO"], section: "geral", funcao: "comercial", mobileTab: true },
  { label: "Leads", href: "/dashboard/comercial/leads", icon: User, roles: ["COMERCIAL", "PARCEIRO"], funcao: "comercial", mobileTab: true },
  ...
  { label: "Perfil", href: "/dashboard/perfil", icon: User, roles: ["GESTOR", "GESTOR_FUNDO", "ENGENHEIRO", "GESTOR_OBRA", "COMERCIAL", "PARCEIRO", "ADMIN", null] },
  ...
  { label: "Liberar etapas", href: "/dashboard/gestor/etapas", icon: ShieldCheck, roles: ["ADMIN"] },
...
export const ROLE_META: Record<string, { label: string; accent: string }> = {
  ...
  TOMADOR: { label: "Tomador", accent: DASHBOARD_MINT },
...
export function getNavRole(role: DashboardUserRole, path: string): DashboardUserRole {
  ...
  if (["inicio", "construtor", "credito", "kyc", "score", "simulador"].includes(seg)) return "CONSTRUTOR";
...
export function getParentPath(path: string): Route | null {
  ...
  if (parts[1] === "inicio" && parts.length > 2) {
    return TOMADOR_HOME;
  }

  if (parts.length === 2) {
    const seg = parts[1];
    if (["inicio", "admin", "gestor", "engenheiro", "comercial"].includes(seg)) {
      return null;
    }
    if (["obras", "notificacoes", "perfil"].includes(seg)) {
      return TOMADOR_HOME;
    }
    return null;
  }

  if (parts[1] === "obras" && parts[2] === "nova") {
    return TOMADOR_HOME;
  }
...
export function isNavActive(path: string, href: string): boolean {
  if (href === TOMADOR_HOME) {
    return path === href || path.startsWith("/dashboard/inicio/");
  }
