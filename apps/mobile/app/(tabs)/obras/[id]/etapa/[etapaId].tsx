...
export default function EtapaEvidenciaScreen() {
  ...
  const [obraData, setObraData] = useState<{ geoLatitude: number; geoLongitude: number; raioValidacaoMetros: number } | null>(null);

  // Load obra data if not provided in params
  useState(() => {
    if (!params.geoLat && params.id) {
  ...
  const geoLat = params.geoLat ? Number(params.geoLat) : obraData?.geoLatitude ?? 0;
  const geoLng = params.geoLng ? Number(params.geoLng) : obraData?.geoLongitude ?? 0;
  const raio = params.raio ? Number(params.raio) : obraData?.raioValidacaoMetros ?? 100;
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      <Text style={styles.title}>{params.etapaNome ?? "Etapa"}</Text>
