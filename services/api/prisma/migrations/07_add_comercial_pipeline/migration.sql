-- Migration: add FCM tokens, ComercialRole, and full comercial pipeline
-- Models: UsuarioFcmToken, PipelineStage, Lead, ConversionScore, LeadActivity
...
-- ── ComercialRole column on Usuario ─────────────────────────────────────

ALTER TABLE "Usuario"
  ADD COLUMN IF NOT EXISTS "comercialRole" "ComercialRole";
...
CREATE TABLE "PipelineStage" (
  "stageId"          TEXT      NOT NULL DEFAULT gen_random_uuid()::text,
  "nome"             TEXT      NOT NULL,
...
CREATE TABLE "Lead" (
  "leadId"               TEXT      NOT NULL DEFAULT gen_random_uuid()::text,
  "obraId"               TEXT,
  "usuarioId"            TEXT,
...
ALTER TABLE "Lead"
  ADD CONSTRAINT "Lead_obraId_fkey"
  FOREIGN KEY ("obraId") REFERENCES "Obra"("obraId")
  ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE "Lead"
  ADD CONSTRAINT "Lead_usuarioId_fkey"
  FOREIGN KEY ("usuarioId") REFERENCES "Usuario"("usuarioId")
  ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE "Lead"
  ADD CONSTRAINT "Lead_stageId_fkey"
  FOREIGN KEY ("stageId") REFERENCES "PipelineStage"("stageId")
  ON DELETE RESTRICT ON UPDATE CASCADE;

-- ── ConversionScore ──────────────────────────────────────────────────────

CREATE TABLE "ConversionScore" (
  "scoreId"               TEXT      NOT NULL DEFAULT gen_random_uuid()::text,
...
ALTER TABLE "ConversionScore"
  ADD CONSTRAINT "ConversionScore_leadId_fkey"
...
CREATE TABLE "LeadActivity" (
  "atividadeId" TEXT      NOT NULL DEFAULT gen_random_uuid()::text,
  "leadId"      TEXT      NOT NULL,
...
INSERT INTO "PipelineStage" ("stageId", "nome", "ordem", "descricao", "corHex", "taxaConversao", "diasMedioStage", "atualizadoEm")
VALUES
  (gen_random_uuid()::text, 'PROSPECÇÃO',     1, 'Leads iniciais identificados',       '#6B7280', 0.80, 3,  NOW()),
  (gen_random_uuid()::text, 'QUALIFICAÇÃO',   2, 'Lead validado e qualificado',         '#3B82F6', 0.60, 5,  NOW()),
  (gen_random_uuid()::text, 'PROPOSTA',       3, 'Proposta comercial enviada',          '#F59E0B', 0.40, 7,  NOW()),
  (gen_random_uuid()::text, 'NEGOCIAÇÃO',     4, 'Em negociação de condições',          '#8B5CF6', 0.25, 10, NOW()),
  (gen_random_uuid()::text, 'FECHAMENTO',     5, 'Contrato assinado',                   '#10B981', 0.10, 2,  NOW()),
  (gen_random_uuid()::text, 'PÓS-VENDA',      6, 'Cliente ativo em acompanhamento',     '#059669', 1.00, 30, NOW())
