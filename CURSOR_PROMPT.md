...
8. Commit to: `claude/imobi-mvp-fintech-status-jrr2ab` (or `main` if branch not available)
...
**Location**: `imobi/ARCHITECTURE_RESILIENCE_API_FIRST.md`
...
**Location**: `imobi/COLLABORATIVE_WORKSPACE.md`
...
| Issue | Solution |
|-------|----------|
| `pnpm type-check` fails with "Cannot find module" | Run `pnpm install` then `pnpm db:generate` |
| `pnpm dev` doesn't start the API | Check terminal output. Default API: `http://localhost:4000` |
| Tailwind styles not applying | Ensure file path is in `apps/web/tailwind.config.ts` content patterns |
| Zod validation errors | Check `packages/schemas/src/*.ts`. Use `z.parse()` for strict validation |
| API returns 401 Unauthorized | Login first; check auth token in cookie |
...
| What | Where | Command |
|------|-------|---------|
| Start dev | Terminal | `pnpm dev` |
| Check types | Terminal | `pnpm type-check` |
